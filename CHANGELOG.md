...
- **`web-static/` — the deterministic pipeline ported to TypeScript, runnable with zero
  ...
  **cross-language conformance test** (`web-static/test/conformance.test.ts`, fixtures
  ...
  `eval/suites/*.yaml` (128 cases across groundedness/safety/refusal/calibration/
  ...
  minimal static demo page (`web-static/public/`) with a web-app manifest and a
  cache-first service worker shows it working end to end; see `web-static/README.md`
  for what's shipped versus deferred (full WCAG/Lighthouse audit, PWA icon assets,
  subresource integrity, and a CI deploy pipeline are follow-up work, not claimed here).
