...
| V1.2 Component access | Private engine internals not reached through a public seam | Deferred | `readyz`/`health` keep the pre-existing `len(engine._store)` read; introducing a public accessor touches the tuning-scope gate's tunable surface (`answer.py`) for a purely cosmetic seam, so it is deliberately left out of this hardening change |
