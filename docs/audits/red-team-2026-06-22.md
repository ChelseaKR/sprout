...
- **LLM10 Unbounded consumption:** offline mode has no per-token cost; Claude generation refuses
  before invocation when its shared-table estimate is unpriced or exceeds
  `max_cost_usd` (0.05/answer), alongside the `max_question_chars` input bound (500). Titan
  embeddings use the exact shared AWS catalog rate for the configured region and reject an
  unsupported or missing region.
...
- Defenses in code: [`src/sprout/guards.py`](https://github.com/ChelseaKR/sprout/blob/main/src/sprout/guards.py),
  [`src/sprout/answer.py`](https://github.com/ChelseaKR/sprout/blob/main/src/sprout/answer.py),
  [`src/sprout/confidence.py`](https://github.com/ChelseaKR/sprout/blob/main/src/sprout/confidence.py)
- Eval suites: [`src/sprout/eval/suites/`](https://github.com/ChelseaKR/sprout/tree/main/src/sprout/eval/suites/)
- Checklist source: `STANDARDS/AI-EVALUATION-STANDARD.md`
