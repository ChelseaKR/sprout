...
| EN/ES pass-rate parity (aggregate delta between languages) | **\|EN − ES\| ≤ 5 pp** (planned) | nothing computes it | **not implemented — corrected 2026-08-29: this row declared AUTO and named the bilingual slice, but `eval/suites/multilingual` gates *per-case* structural parity at ≥ 0.85 (the row above), which is a different quantity from an EN-vs-ES pass-rate delta; no code computes that delta (no `parity_gap`, `pass_rate_parity` or `en_es` symbol exists anywhere under `src/`) and `docs/cards/model-card.md` already records `en-es-parity` as `value: null, verified: false`. Implementing it means scoring the EN anchors as their own slice and comparing pass rates — tracked, not claimed.** |
...
| EN/ES eval pass-rate parity (aggregate delta between languages) | **\|EN − ES\| ≤ 5 pp** (planned) | nothing computes it | **not implemented — corrected 2026-08-29; see the identically-corrected row in the AI-evaluation ledger above** |
...
| Internationalization | APPLIES | EN/ES key + placeholder parity; per-case EN/ES structural parity gated at ≥ 0.85 by `eval/suites/multilingual` (an aggregate \|EN − ES\| pass-rate delta is planned, not implemented) |
