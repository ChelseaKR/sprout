...
it. Each row is labeled `risk @ confidence≥T (coverage C)`: `T` is the confidence cutoff
and `C` the coverage measured at it. Labeling the row `coverage≥T` instead — as the first
